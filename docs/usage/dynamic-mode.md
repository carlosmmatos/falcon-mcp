...
<!-- meta:description Reduce context usage with discovery, execution, inventory, and bounded result retrieval. -->
<!-- meta:section usage -->
...
Dynamic mode solves this by replacing the full tool surface with four tools. Two are the
...
The fourth tool, `falcon_read_result`, retrieves oversized saved results in bounded
pieces within the same session. Pass `response_options` on `falcon_execute_tool`,
outside its `parameters` object, to select `summary`, `compact`, `full`, fields,
or a smaller byte budget. See [response budgets](response-budgets.md).
...
With dynamic mode enabled, the server exposes four tools instead of the full module surface —
...
| `falcon_read_result` | Inspect a saved oversized result in bounded pieces within the same session |
